    ...
    if (!externalResponse.ok) {
      ...
      if (tool.transformError) {
        try {
          ...
          throw new Error(
            `${toolId} API error: ${externalResponse.statusText}`
          );
        }
      } else {
        const error =
...
export function validateToolRequest(
  ...
  for (const [paramName, paramConfig] of Object.entries(tool.params)) {
    if (paramConfig.requiredForToolCall && !(paramName in params)) {
      throw new Error(
        `Parameter "${paramName}" is required for ${toolId} but was not provided`
      );
    }
  }
}
...
export function createParamSchema(customTool: any): Record<string, any> {
  ...
  if (customTool.schema.function?.parameters?.properties) {
    ...
    const optionalToolInputs =
      customTool.schema.function.parameters.optionalToolInputs || [];
      ...
      if (isFreestyleAvailable()) {
        try {
          ...
          for (const match of envVarMatches) {
            ...
            const varValue = envVar
              ? envVar.value
              : mergedParams[varName] || "";
          ...
          logger.warn(
            "Freestyle execution failed, falling back to API:",
            error.message
          );
...
async function getCustomTool(
  ...
  try {
    ...
    if (!result.data || !Array.isArray(result.data)) {
      logger.error(
        `Invalid response when fetching custom tools: ${JSON.stringify(result)}`
      );
      return undefined;
    }
